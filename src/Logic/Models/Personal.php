...
class Personal {
    ...
    public static function buscarPorPersona($id_persona) {
        $conn = Database::getConnection();
        $query = "SELECT * FROM personal WHERE id_persona = :id_persona";
        $stmt = $conn->prepare($query);
    ...
    public static function crear($id_persona, $id_especialidad, $matricula) {
        $conn = Database::getConnection();
        $query = "INSERT INTO personal (id_persona, id_especialidad, matricula_profesional) 
                  VALUES (:id_persona, :id_especialidad, :matricula)";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(":id_persona", $id_persona);
